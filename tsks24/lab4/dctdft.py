...
# Updated by Maria Magnusson 2022-03-02
#-----------------------------------------------------------------------

import numpy as np
from matplotlib import pyplot as plt
import cv2
a = np.load('goldhill128.npy')			# inimage
N = 128						# inimage size
...
plt.title('abs(AA(u,v) = FFT[a(x,y)])'), plt.colorbar()
plt.subplot(223), plt.imshow(np.abs(A), 'gray', clim=FLIM)
plt.title('abs(A(u,v) = DCT[a(x,y)])'), plt.colorbar()
...
plt.title('abs(B(u,v) = almost FFT[b(x,y)])'), plt.colorbar()
BPART = B[N:,N:]
plt.subplot(224), plt.imshow(np.abs(BPART), 'gray', clim=FLIM)
plt.title('lower right part of abs(B(u,v))'), plt.colorbar()

print(f"Maximum difference: {np.max(A-BPART)}")              # check that they are equal
print(f'Minimum difference: {np.min(A-BPART)}')              # check that they are equal
